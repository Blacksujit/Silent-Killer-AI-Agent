...
const CleanDashboard = ({ stats, setStats }) => {
  ...
  const hoveredChartRef = useRef(null)

  const systemColorClass = (color) => {
    switch (color) {
      case 'cyan':
        return {
          icon: 'text-cyan-400',
  ...
  const showNotification = (title, message, type = 'info') => {
    ...
    const now = Date.now()
    if (now - lastNotificationTime < 30000) return // 30 second cooldown
    
    setLastNotificationTime(now)

    toast.dismiss('dashboard-toast')
  ...
  // Initialize with stable, non-flickering data
  useEffect(() => {
    const now = new Date()
  ...
  useEffect(() => {
    ...
    const tick = async () => {
      try {
        ...
        // compute a stable rate based on change in event_count over time
        const now = Date.now()
        ...
        if (typeof prevCount === 'number') {
          rate = Math.max(0, (count - prevCount) / dt)
        }

        const changed = prevCount !== count || prevTs !== ts
        if (changed) {
          setStats((prev) => ({
            ...prev,
            events: count,
          ...
          setLastBackendStatsAt(new Date())
        }
    ...
    <div className="space-y-6">
      ...
      >
        <div>
          ...
          <p className="text-gray-400">Stable monitoring with non-flickering charts</p>
          <div className="mt-2">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3 sm:gap-4">
              ...
              <div className="flex items-center justify-between">
                ...
                >
                  ...
                  <option value="30000">30s (Recommended)</option>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-4 lg:gap-6">
        ...
        <motion.div
          ...
          onHoverStart={() => {
            ...
            hoveredChartRef.current = 'timeline'
          }}
          onHoverEnd={() => {
        ...
        >
          <div className="flex items-center justify-between mb-4">
            <div className="flex items-center gap-3">
              <h3 className="text-lg font-semibold text-cyan-400">Activity Timeline</h3>
        ...
        <motion.div
          ...
          onHoverStart={() => {
            ...
            hoveredChartRef.current = 'distribution'
          }}
          onHoverEnd={() => {
        ...
        >
          <div className="flex items-center justify-between mb-4">
            <div className="flex items-center gap-3">
              <h3 className="text-lg font-semibold text-purple-400">Activity Distribution</h3>
      ...
      >
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 lg:gap-6">
          {[
            ...
            const colors = systemColorClass(item.color)
            return (
      ...
      >
        <div className="flex items-center justify-between mb-4">
          <h3 className="text-lg font-semibold text-cyan-400">Event Log</h3>
