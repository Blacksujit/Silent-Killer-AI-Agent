...
const PerfectDashboard = ({ stats, setStats }) => {
  // Core state
  ...
  const [refreshInterval, setRefreshInterval] = useState(20000) // 20 seconds
  ...
  // Perfect notification system with proper alignment
  const showNotification = (title, message, type = 'info') => {
    ...
    switch (type) {
      case 'warning':
        toastOptions.style.border = '1px solid rgba(255, 0, 102, 0.5)'
        toastOptions.style.color = '#ff0066'
        toastOptions.iconTheme.primary = '#ff0066'
        toast.error(`${title}: ${message}`, toastOptions)
        break
      case 'success':
        toastOptions.style.border = '1px solid rgba(0, 255, 136, 0.5)'
        toastOptions.style.color = '#00ff88'
        toastOptions.iconTheme.primary = '#00ff88'
  ...
  // Initialize data with stable values
  useEffect(() => {
    const now = new Date()
    const initialChartData = Array.from({ length: 24 }, (_, i) => {
      ...
      return {
        ...
        events: Math.floor(Math.sin(i / 4) * 10 + 100),
        suggestions: Math.floor(Math.cos(i / 6) * 2 + 12),
        cpu: Math.sin(i / 8) * 8 + 50,
        memory: Math.cos(i / 10) * 6 + 60
      }
    })
  ...
  // Main update loop with proper animation control
  useEffect(() => {
    if (!autoRefresh || isPaused) return
    
    let lastUpdate = 0
    const updateInterval = 1000 / 2 // 2 FPS for very slow, stable updates
    ...
    const updateData = (timestamp) => {
      if (timestamp - lastUpdate >= refreshInterval) {
        ...
        setChartData(prev => prev.map(item => ({
          ...item,
          events: item.events + (Math.random() > 0.97 ? (Math.random() > 0.5 ? 1 : -1) : 0),
          suggestions: item.suggestions + (Math.random() > 0.99 ? (Math.random() > 0.5 ? 1 : -1) : 0),
          cpu: Math.max(0, Math.min(100, item.cpu + (Math.random() > 0.95 ? (Math.random() - 0.5) * 1 : 0))),
          memory: Math.max(0, Math.min(100, item.memory + (Math.random() > 0.95 ? (Math.random() - 0.5) * 1 : 0)))
        })))

        // Update real-time stats very slowly
        setRealtimeStats(prev => {
          const newCpu = Math.max(0, Math.min(100, prev.cpu + (Math.random() - 0.5) * 0.5))
          const newMemory = Math.max(0, Math.min(100, prev.memory + (Math.random() - 0.5) * 0.5))
          const newEvents = prev.events + (Math.random() > 0.99 ? 1 : 0)
          const newUptime = prev.uptime + 1
          const newNetwork = Math.max(0, prev.network + (Math.random() - 0.5) * 1)
        ...
        setStats(prev => ({
          ...prev,
          events: prev.events + (Math.random() > 0.99 ? 1 : 0),
          rate: prev.rate + (Math.random() - 0.5) * 0.005
        }))
        
        // Trigger animation key change for pie chart
  ...
  }, [stats, setStats, autoRefresh, refreshInterval, isPaused])

  // Export functions with proper notifications
  const exportData = () => {
    const data = {
    ...
    showNotification('Export Complete', 'Dashboard data exported as JSON', 'success')
  }
  ...
  const exportCSV = () => {
    ...
    showNotification('Export Complete', 'Dashboard data exported as CSV', 'success')
  }
        ...
        <div className="bg-gray-900/95 backdrop-blur-md border border-cyan-500/30 rounded-lg p-3 shadow-lg">
          ...
          {payload.map((entry, index) => (
            <p key={index} className="text-cyan-200 text-sm">
              <span className="text-cyan-400">{entry.name}:</span> {entry.value.toFixed(1)}
            </p>
          ))}
        </div>
      )
  ...
  return (
    <div className="space-y-6">
        ...
        <div className="flex flex-wrap items-center gap-3">
          ...
          <motion.button
            ...
            onClick={() => {
              ...
              showNotification('Auto Refresh', !isPaused ? 'Resumed' : 'Paused', 'info')
            }}
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
              ...
              <div className="flex items-center justify-between">
                ...
                <button
                  onClick={() => {
                    ...
                    showNotification('Notifications', !notifications ? 'Enabled' : 'Disabled', 'info')
                  }}
                  className={`w-12 h-6 rounded-full transition-colors ${
              ...
              <div className="flex items-center justify-between">
                ...
                >
                  ...
                  <option value="20000">20s (Recommended)</option>
      ...
      {/* Enhanced Charts with Proper Animations */}
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        {/* Enhanced Activity Distribution with Perfect Pie Chart Animation */}
        <motion.div
          initial={{ opacity: 0, x: 20 }}
          animate={{ opacity: 1, x: 0 }}
          whileHover={{ 
